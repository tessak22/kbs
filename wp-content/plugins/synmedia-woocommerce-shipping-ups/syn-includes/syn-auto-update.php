...
class SYN_Auto_Update
{
     ...
     * @var string
     */
    public $update_path = 'http://api.synmedia.ca';
    /**
	...
	public function check_info( $def, $action, $args ){
		...
		if ( is_wp_error( $raw_response ) )
			return new WP_Error('plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with api.synmedia.ca or this server&#8217;s configuration. If you continue to have problems, please try to <a href="http://www.synmedia.ca">contact us</a>.' ), $raw_response->get_error_message() );
			
		$response = (object) json_decode( wp_remote_retrieve_body( $raw_response ), true );
			
		if ( !is_object( $response ) )
			return new WP_Error('plugins_api_failed', __( 'An unexpected error occurred. Something may be wrong with api.synmedia.ca or this server&#8217;s configuration. If you continue to have problems, please try to <a href="http://www.synmedia.ca">contact us</a>.' ) );
